package com.pelni.boarding.ticket.util;

public class MyConstants {
    public static final String MESSAGE = "message";
    public static final String TIMESTAMP = "timestamp";
    public static final String STATUS_CODE = "statusCode";
    public static final String STATUS_MESSAGE = "statusMessage";
    public static final String SUCCESS = "Success";
    public static final String DATA = "data";

    public static final String ADULT = "Dewasa";
    public static final String MALE = "PRIA";
    public static final String FEMALE = "WANITA";
    public static final String INFANT = "Bayi";
    public static final String PROD = "PROD";
    public static final String DEMO = "DEMO";
    public static final String USER = "USER";
    public static final String LOGIN = "LOGIN APLIKASI";
    public static final String INVALID_PASSWORD = "Invalid Password";
    public static final String INVALID_USERNAME = "Invalid Username";
    public static final String GET_CHECKIN_INFO = "GET-CHECKIN-INFO (%s)";
    public static final String FLAGING_CHECKIN = "FLAGING-CHECKIN (%s) / (%s)";
    public static final String LIMIT = "You have exceeded the ticket printing time limit.";

}
